...
if (typeof AttachmentExtractor == "undefined") {
	...
	AttachmentExtractor.prototype.init = function() {
		...
		this.prefs.aeBranch.QueryInterface(Ci.nsIPrefBranch2).addObserver("savepathmru", this.prefObserver, false);
		this.prefs.aeBranch.QueryInterface(Ci.nsIPrefBranch2).addObserver("defaultsavepath", this.prefObserver, false);
		this.prefs.aeBranch.QueryInterface(Ci.nsIPrefBranch2).addObserver("autoextract.savepath", this.prefObserver, false);
...
// 	
	...
	AttachmentExtractor.prototype.openAEDialog = function(savefolder,messages,filenamepattern,background,deleteAtt) {
		return window.openDialog("chrome://attachmentextractor/content/aeDialog.xul",
	...
	AttachmentExtractor.prototype.getSaveFolder=function(pref,updatepref) {
		...
		try {
			 fp.init(window, this.aeStringBundle.GetStringFromName("FolderPickerDialogTitle"), Ci.nsIFilePicker.modeGetFolder);
			 try {
				var initialDir = this.prefs.hasUserValue(pref,"")? this.prefs.getFile(pref,"") : null;
				if (initialDir) fp.displayDirectory = initialDir;
			 } catch (e) {aedump(e,1);}
			 var r = fp.show();
			 if (r == Ci.nsIFilePicker.returnCancel) return false;
		} catch(e) {aedump(e,0);}
		if (updatepref) this.prefs.setFile(pref, fp.file,"");
		return fp.file;
	};
	...
	AttachmentExtractor.prototype.getSuggestedSaveFolder=function(messages) {
		...
		if (!this.prefs.hasUserValue("suggestfolder.parent.1")) {
			...
			ww.openWindow(null, 'chrome://attachmentextractor/content/settings/general_suggest.xul',"", 'chrome,resizable,toolbar,modal', null);
		}
		...
		while (ps.prefHasUserValue("suggestfolder.parent."+pfc)) {
		  try {
			var folder=ps.getComplexValue("suggestfolder.parent."+pfc, Ci.nsILocalFile);
		...
		window.openDialog("chrome://attachmentextractor/content/suggestedDialog.xul", "",
	...
	AttachmentExtractor.prototype.addHeaderButton=function() {
	  try{
		...
		junk=document.getElementById("collapsedButtonBox")
		if (junk) junk=junk.getButton('hdrJunkButton');
		else return;
