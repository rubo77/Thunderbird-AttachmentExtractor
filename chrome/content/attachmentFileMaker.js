 ...
 *
 * Contributor(s): Matteo F Zazzetta 
 * (bug fixes, contributed complete rewrite of validateFileName and supporting 
 *	functions to handle long filenames and inspired rewrite of cleanSubjectLine)
 *
...
dump("##AttachmentFileMaker Prototype Definition start##\n");
	...
	AttachmentFileMaker.prototype.make=function (filename,metacache) {	
		...
		if (!proposedfileobject.exists()) {
			aewindow.aedump(">> '"+proposedappendage+"' doesn't exist so proceed\n",1);
			this.lastMadeAppendage=proposedappendage;
			return proposedfileobject;
		}
	...
	AttachmentFileMaker.prototype.makeSaveMessage=function(metacache) {
		...
		var count_pattern=this.prefs.get("filenamepattern.savemessage.countpattern");
		
		proposedfileobject.initWithFile(this.destFolder);
		
		var proposedappendage = this.substitutetokens(messagetext_pattern.replace(/#count#/g,""),this.destFolder,"",0,metacache);
		proposedfileobject.appendRelativePath(proposedappendage);
		...
		else return this.iterativegenerator(this.destFolder,"",proposedfileobject,messagetext_pattern.replace(/#count#/g,count_pattern),1,metacache);
	};
	...
	AttachmentFileMaker.prototype.DAYSUFFIX= Array("",
							 ...
							 "st","nd","rd","th","th","th","th","th","th","th",
							 "st");
	AttachmentFileMaker.prototype.SHORTDAYS= Array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");
	AttachmentFileMaker.prototype.FULLDAYS= Array("Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday");
	AttachmentFileMaker.prototype.SHORTMONTHS=Array("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec");
	...
	AttachmentFileMaker.prototype.formatdatestring=function(format,date) {
		...
		for (var i=0;i<format.length;i++) {
			switch (format.charAt(i)) {
				//day
				case "d": out+=(tmp=date.getDate())<10?"0"+tmp:tmp; break;
				case "D": out+=this.SHORTDAYS[date.getDay()]; break;
				case "j": out+=date.getDate(); break;
				case "l": out+=this.FULLDAYS[date.getDay()]; break;
				case "N": out+=(tmp=date.getDay())==0?7:tmp; break;
				case "S": out+=this.DAYSUFFIX[date.getDate()]; break;
				...
				case "y": out+=(tmp=date.getFullYear() % 100)<10?"0"+tmp:tmp; break;
				//time
				case "a": out+=this.AMPM[(date.getHours()<12?2:3)]; break;
				case "A": out+=this.AMPM[(date.getHours()<12?0:1)]; break;
				case "g": out+=(tmp=date.getHours() % 12)==0?12:tmp; break;
				case "G": out+=date.getHours(); break;
				case "h": out+=(tmp=((tmp=date.getHours() % 12)==0?12:tmp))<10?"0"+tmp:tmp; break;
				case "H": out+=(tmp=date.getHours())<10?"0"+tmp:tmp; break;
				case "i": out+=(tmp=date.getMinutes())<10?"0"+tmp:tmp; break;
				case "s": out+=(tmp=date.getSeconds())<10?"0"+tmp:tmp; break;
				//full date/time
				case "U": out+=date.valueOf()/1000; break;
				default : out+=format.charAt(i);
			}
		}
		return out;
	};
	  ...
	  // <?php
	  /*function recursivegenerator(folder,file,proposedfileobject,filename_pattern,count,cache)
	  {
		proposedfileobject.initWithFile(folder);
		var proposedappendage=substitutetokens(filename_pattern,file,count,cache);
		proposedfileobject.appendRelativePath(proposedappendage);
		//aewindow.aedump("proposed file: "+proposedfileobject.path+"\n");
		if (proposedfileobject.exists()) return recursivegenerator(folder,file,proposedfileobject,filename_pattern,count+1,cache);
		aewindow.aedump(">> '"+proposedappendage+"' doesn't exist so proceed\n",1);
		return proposedfileobject;
	  };*/
	  // ?>
	...
	AttachmentFileMaker.prototype.iterativegenerator=function(folder,file,proposedfileobject,filename_pattern,count,cache) {
		...
		do {
			proposedfileobject.initWithFile(folder);
			proposedappendage=this.substitutetokens(filename_pattern,folder,file,count,cache);
			proposedfileobject.appendRelativePath(proposedappendage);
			count++;
		} while (proposedfileobject.exists())
		if (file!="") aewindow.aedump(">> '"+proposedappendage+"' doesn't exist so proceed\n",1);
		this.lastMadeAppendage=proposedappendage;
		return proposedfileobject;
	};
	...
	AttachmentFileMaker.prototype.substitutetokens_sub=function(stringin,folder,filename,count,cache) {
		...
		if ((stringin.indexOf("#namepart#")> -1) || (stringin.indexOf("#extpart#")> -1)) {
			...
			stringin=stringin.replace(/#namepart#/g,namepart.replace(this.tokenregexs.dollars,"$$$$"));
		}
	...
	AttachmentFileMaker.prototype.generate=AttachmentFileMaker.prototype.substitutetokens_sub;
	...
	AttachmentFileMaker.prototype.substitutetokens=function(stringin, folder,filename,count,cache) {
		...
		if (!cache.datetime&&(stringin.indexOf('#date#')>-1)) {
				cache.datetime=this.formatdatestring(this.prefs.get("filenamepattern.datepattern"),new Date(msheader.dateInSeconds*1000)); 	
		} 
		if (!cache.author&&(stringin.indexOf('#from#')>-1)) {
			cache.author=(msheader.mime2DecodedAuthor?msheader.mime2DecodedAuthor+"":"").replace(this.tokenregexs.author, "")
		...
		if (!cache.subject&&(stringin.indexOf('#subject#')>-1)) {
			cache.subject=(msheader.mime2DecodedSubject?msheader.mime2DecodedSubject+"":"").replace(this.tokenregexs.subject, "_")
																						   .replace(this.tokenregexs.dollars,"$$$$");
			if (this.prefs.get("filenamepattern.cleansubject")) {
				var starts=this.prefs.get("filenamepattern.cleansubject.strings").toLowerCase().split(',');
				cache.subject=this.cleanSubjectLine(cache.subject,starts);
			}
		}
		if (!cache.mailfolder&&(stringin.indexOf('#folder#')>-1)) {
			cache.mailfolder=(msheader.folder?msheader.folder.name+"":"").replace(this.tokenregexs.folder, "")
																		 .replace(this.tokenregexs.dollars,"$$$$");
		}
		
		aewindow.aedump("cache: "+cache+"\n",3);
		//aewindow.aedump("m2da: "+msheader.mime2DecodedAuthor+", a: "+msheader.author+"\n",0);
		return this.substitutetokens_sub(stringin, folder,filename, count, cache);
	};
	  
	  // <?php
	  /*function cleanSubjectLine(subject,starts) {
		  if (subject.charAt(0)=='[' && subject.charAt(subject.length-1)==']') return cleanSubjectLine(subject.substring(1,subject.length-1),starts);
		  for (var i = 0; i < starts.length; i++) {
				if (subject.toLowerCase().indexOf(starts[i])==0) return cleanSubjectLine(subject.substring(starts[i].length),starts);
		  }
		  return subject;
	  };*/
	  // ?>
	...
	AttachmentFileMaker.prototype.fixFilenamePattern=function(fnp) {
		  ...
		  if (expi!=-1) return fnp.substring(0,expi)+"#count#"+fnp.substring(expi,fnp.length);
	...
	AttachmentFileMaker.prototype.countOccurrences=function(string,substring,rep) {
		  
		  function StrIdx(i,s,r) {
			  this.idx = i;
			  this.string = s; 
			  this.replace = r;
			  this.toString = function() {return this.idx+" "+this.string+" "+this.replace;};
		  };
	...
	AttachmentFileMaker.prototype.reconstructString=function(string,idxarr) {
		  ...
		  for (var i=0; i<idxarr.length; i++) {
			...
			if (pos<rs.length) { 
			  rs = (pos>0?rs.substring(0,pos):"")+idxarr[i].replace+rs.substring(pos);
			}
			else {
			  rs+=idxarr[i].replace;
			}
	...
	AttachmentFileMaker.prototype.validateFileName=function(folder,aFileName,extpart,count) {
		  
		  function joinArrs(a1,a2) {
			return a1.concat(a2).sort(function(a,b) {return a.idx-b.idx;});
		  }
		  ...
		  if (navigator.appVersion.indexOf("Windows") != -1) {
			aFileName = aFileName.replace(/[\"]+/g, "'")			/*  " = '			*/
								 ...
								 .replace(/[\>]+/g, ")")			/*  > = )			*/
								 .replace(/[\/\|]+/g, "_");        	/*  /| = _ 		    */
			var comp = (folder==null||folder.path==null?"":folder.path) + "\\" + aFileName;
			comp = comp.replace(/\\/g,"");
			var len1 = comp.length;
			var len2 = comp.replace(/%/g,count).replace(/#extpart#/g,extpart.replace(this.tokenregexs.dollars,"$$$$")).length;
			...
			if (len2 >= 250) {
				...
				if (charsToDelete<=totlen) {
					...
					for (var i=0; i<splits.length; i++) {
							var arr = joinArrs(this.countOccurrences(splits[i],"%",count),this.countOccurrences(splits[i],"#extpart#",extpart));
							
							splits[i] = splits[i].replace(/%|#extpart#/g,"");
							cd = (i==splits.length-1)?remainToDelete:Math.min(Math.ceil((splits[i].length-1)/totlen*charsToDelete),splits[i].length-1);
							remainToDelete-=cd;
							splits[i] = this.reconstructString(splits[i].substring(0,splits[i].length-cd),arr);
							newfn+=(i==0?"":"\\")+splits[i];
					}
					aFileName = newfn;
				}
			}
			else aFileName = aFileName.replace(/%/g,count).replace(/#extpart#/g,extpart.replace(this.tokenregexs.dollars,"$$$$"));
			aFileName = aFileName.replace(/[\. ]+\\/g,"\\");
		  } 
		  else {
			if (navigator.appVersion.indexOf("Macintosh") != -1) aFileName = aFileName.replace(/[\:]+/g, "_");		/*   : = _ 	*/
			aFileName = aFileName.replace(/%/g,count).replace(/#extpart#/g,extpart.replace(this.tokenregexs.dollars,"$$$$"));
		  }
		  return aFileName;
	}  
